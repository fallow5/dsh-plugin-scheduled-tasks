...
describe("TaskScheduler", () => {
	...
	it("does not dispatch a task whose effectiveFrom is still in the future", async () => {
		const store = new FakeStore();
		const task = makeTask({
			id: "task-1",
			scheduledAt: "2026-08-14T08:00:00.000Z",
	...
	it("finishes a task whose effectiveUntil has passed", async () => {
		const store = new FakeStore();
		const task = makeTask({
			id: "task-1",
			scheduledAt: "2026-08-14T08:00:00.000Z",
			effectiveUntil: "2026-08-13T23:59:59.999Z",
		});
		store.tasks.set(task.id, task);
		const executor = new FakeExecutor();
		const scheduler = new TaskScheduler(
		...
		expect(executor.runs).toHaveLength(0);
		expect(store.tasks.get("task-1")!.state).toBe("finished");
		expect(store.tasks.get("task-1")!.enabled).toBe(false);
	});

	it("dispatches a task that is within its effective date range", async () => {
		const store = new FakeStore();
		const task = makeTask({
			id: "task-1",
			scheduledAt: "2026-08-14T08:00:00.000Z",
